...
class ShrewdEyeEnum(enumratorBaseThreaded):
    def __init__(self, domain, subdomains=None, q=None, silent=False, verbose=True):
        super(ShrewdEyeEnum, self).__init__(None, "ShrewdEye", domain, subdomains, q=q, silent=silent, verbose=verbose)
        self.base_url = 'https://shrewdeye.app/domains/{domain}.txt'
    ...
    def enumerate(self):
        ...
        try:
            ...
            if response.status_code == 200:
                for subdomain in response.text.splitlines():
                    if subdomain.endswith('.' + self.domain):
                        if self.verbose and subdomain not in self.subdomains:
                            self.print_("%s%s: %s%s" % (R, self.engine_name, W, subdomain))
                        self.subdomains.append(subdomain.strip())
            else:
                if self.verbose:
                    self.print_("%s[!] Error: Unable to retrieve ShrewdEye data%s" % (R, W))
        except Exception as e:
            if self.verbose:
                self.print_("%s[!] Error: %s%s" % (R, e, W))
        return self.subdomains
...
class Virustotal(enumratorBaseThreaded):
    def __init__(self, domain, subdomains=None, q=None, silent=False, verbose=True, apikey=None):
        subdomains = subdomains or []
        self.apikey = apikey or os.environ.get('VTAPIKEY')
        if not self.apikey:
            print("VTAPIKEY environment variable is not set.")
            self.apikey = self.prompt_for_api_key()
        self.base_url = 'https://www.virustotal.com/vtapi/v2/domain/report'
        self.engine_name = "Virustotal"
        self.q = q
        super(Virustotal, self).__init__(self.base_url, self.engine_name, domain, subdomains, q=q, silent=silent, verbose=verbose)

    def prompt_for_api_key(self):
        api_key_pattern = re.compile(r'^[a-f0-9]{64}$')
        while True:
            apikey = input('\033[91m' + "Please enter your VirusTotal API Key: " + '\033[91m').strip()
            if api_key_pattern.match(apikey):
                return apikey
            else:
                print('\033[91m' + "Invalid API Key format. The key should be a 64-character hexadecimal string." + '\033[0m')

    def send_req(self):
        params = {'apikey': self.apikey, 'domain': self.domain}
        try:
            response = self.session.get(self.base_url, headers=self.headers, params=params, timeout=self.timeout)
            if response.status_code != 200:
                raise Exception('\033[91m' + "Error from VirusTotal API: HTTP {}" + '\033[91m'.format(response.status_code))
            return response.json()
        except Exception as e:
            self.print_(e)
            return None
    ...
    def extract_domains(self, resp):
        try:
            for subdomain in sorted(resp.get('subdomains', [])):
                if subdomain not in self.subdomains and subdomain.endswith(self.domain):
                    if self.verbose:
                        self.print_("%s%s: %s%s" % (R, self.engine_name, W, subdomain))
                    self.subdomains.append(subdomain)
        except Exception as e:
            self.print_("Error processing response: {}".format(e))
...
def main(domain, threads, savefile, ports, silent, verbose, enable_bruteforce, engines):
    ...
    if engines is None:
        chosenEnums = [
            ...
            CrtSearch, PassiveDNS, ShrewdEyeEnum
        ]
    else:
